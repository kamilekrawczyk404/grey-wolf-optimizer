...
export function MultiTabTestRunner() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log("Tab became visible - syncing checkpoints");
        syncCheckpoints();
      }
    };
                  ...
                  >
                    {editingTabId === session.id ? (
                      ...
                      <>
                        ...
                        {/* Checkpoint available indicator */}
                        {session.runId && session.status === "idle" && (
                          <Badge
                            variant="outline"
                            className="ml-1 h-4 px-1 text-[10px] bg-orange-900 border-orange-700 text-orange-200"
                          >
                            Resume
                          </Badge>
                        )}
