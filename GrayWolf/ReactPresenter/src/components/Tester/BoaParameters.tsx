...
const ParameterInput = ({ label, value, onChange }: { label: string; value: string | number; onChange: (val: any) => void; }) => (
    <div className="flex flex-col gap-1 flex-1">
        ...
        <input
            className="p-2 rounded bg-neutral-800 text-white w-full border border-neutral-700 focus:border-pink-500 outline-none"
            value={value}
...
const BoaParameters: React.FC = () => {
    ...
    const runOptimizer = async () => {
        ...
        const requestBody = {
            Algorithm: "BOA", // Klucz dla Butterfly Algorithm
    ...
    return (
        <div className="space-y-4">
            <SectionContainer header="Butterfly Optimization Parameters">
                ...
                <button
                    ...
                    disabled={loading}
                    className={`mt-4 px-4 py-2 rounded ${loading ? "bg-neutral-700 cursor-not-allowed" : "bg-pink-600 hover:bg-pink-500 text-white"}`}
                >
                    {loading ? "Running BOA..." : "Run Butterfly Algorithm"}
                </button>
            </SectionContainer>

            {result.Message && <SectionContainer header="Info"><div>{result.Message}</div></SectionContainer>}
            {result.BestSolution && <SectionContainer header="Result"><div className="break-all">Best Solution: {result.BestSolution.join(", ")}</div></SectionContainer>}
            {result.Error && <SectionContainer header="Error"><div className="text-red-500">{result.Error}</div></SectionContainer>}
        </div>
    );
};

export default BoaParameters;