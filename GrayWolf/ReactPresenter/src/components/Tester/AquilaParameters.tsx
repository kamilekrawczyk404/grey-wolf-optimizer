...
const ParameterInput = ({ label, value, onChange }: { label: string; value: string | number; onChange: (val: any) => void; }) => (
    <div className="flex flex-col gap-1 flex-1">
        ...
        <input
            className="p-2 rounded bg-neutral-800 text-white w-full border border-neutral-700 focus:border-orange-500 outline-none" // Inny kolor dla Aquila
            value={value}
...
const ParameterSelect = ({ label, value, onChange, options }: { label: string; value: string; onChange: (val: string) => void; options: string[]; }) => (
    <div className="flex flex-col gap-1">
        <span className="text-xs text-neutral-300 font-medium">{label}</span>
        <select
            className="p-2 rounded bg-neutral-800 text-white w-full border border-neutral-700 focus:border-orange-500 outline-none"
            value={value}
...
const AquilaParameters: React.FC = () => {
    ...
    return (
        <div className="space-y-4">
            <SectionContainer header="Aquila Optimizer Parameters">
                {/* Tutaj możesz w przyszłości dodać parametry specyficzne dla Aquila */}
                ...
                <button
                    ...
                    disabled={loading}
                    className={`mt-4 px-4 py-2 rounded ${loading ? "bg-neutral-700 cursor-not-allowed" : "bg-orange-600 hover:bg-orange-500 text-white"}`}
                >
                    {loading ? "Running Aquila..." : "Run Aquila"}
                </button>
            </SectionContainer>

            {/* Wyniki */}
            {result.Message && (
                <SectionContainer header="Aquila Info">
                    <div>{result.Message}</div>
                </SectionContainer>
            )}
            {result.BestSolution && (
                <SectionContainer header="Aquila Result">
...
export default AquilaParameters;