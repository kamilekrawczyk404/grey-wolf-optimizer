...
export function TestResultsDialog({
  ...
  const getStatusDescription = () => {
    let processedObject: string;

    if (session.result?.type === "single") {
      processedObject = (session.config as SingleTestFormValues).algorithm;
    } else if (session.result?.type === "multi") {
      processedObject = (session.config as MultiTestFormValues)
        .benchmarkFunction;
    } else if (session.result?.type === "function-comparison") {
      processedObject = (session.config as FunctionComparisonFormValues)
        .algorithm;
    } else {
      processedObject = "Unknown";
    }
  ...
  if (session.result.type === "function-comparison") {
    const result = session.result as FunctionComparisonResult;
      ...
      <Dialog open={open} onOpenChange={(val) => !val && handleClose()}>
        <DialogContent className="max-w-5xl bg-neutral-900 border-neutral-800">
          <DialogHeader>
            ...
            <DialogDescription>
              Algorithm: {result.algorithm} | Iterations:{" "}
              {session.config.iterations}
            </DialogDescription>
          </DialogHeader>

          <div className="border border-neutral-700 rounded-md overflow-hidden mt-4">
            <Table>
              <TableHeader className="bg-neutral-800">
                <TableRow className="border-neutral-700 hover:bg-neutral-800">
                  <TableHead className="text-neutral-300">Function</TableHead>
                ...
                {result.results.map((row, idx) => (
                  ...
                  >
                    <TableCell className="font-medium text-white">
                      <Badge
                        variant="outline"
                        className="bg-cyan-600/20 text-cyan-400 border-cyan-600"
                      >
                        {row.benchmarkFunction}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      {row.status === "success" ? (
                    ...
                    <TableCell className="font-mono text-blue-300">
                      <ScrollArea className="h-[120px] min-h-0 rounded-md border border-neutral-700 p-3 bg-neutral-950">
                        <div className="text-sm text-white font-mono leading-relaxed space-y-1">
                          {row.status === "success" && row.bestSolution ? (
